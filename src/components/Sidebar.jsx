...
export default function Sidebar({
  ...
  const createEvent = async () => {
    ...
    try {
      ...
      await (await import("firebase/firestore")).setDoc(doc(db, "events", eid, "crowd", "config"), { regularDevotees: 300, footfallPerKm: 800, routeKm: 3 });
  ...
  const deleteEvent = async (eid) => {
    ...
    if (selectedEventId === eid) {
      setSelectedEventId(events.length > 1 ? events.find(e => e.id !== eid)?.id : null);
    }
  };
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed md:sticky top-0 left-0 h-full md:h-[calc(100vh-64px)] w-72 bg-white border-r border-gray-200 shadow-xl md:shadow-none z-40 transform transition-transform duration-300 overflow-y-auto ${
            ...
            <div className="flex gap-2 mb-4">
              ...
              {selectedEventId && (
                <button
                  ...
                  className="text-xs bg-red-50 text-red-500 hover:bg-red-100 font-bold py-2 px-3 rounded-xl transition-all"
                >
                  🗑️
                </button>
              )}
          ...
          {showCreate && (
            <div className="mb-4 bg-orange-50 rounded-xl p-3 border border-orange-200 space-y-2">
              ...
              <button
                onClick={createEvent}
                disabled={creating}
                className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm py-2 rounded-xl shadow disabled:opacity-50"
              >
